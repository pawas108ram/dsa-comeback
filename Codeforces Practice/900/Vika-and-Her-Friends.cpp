// Vika and her friends went shopping in a mall, which can be represented as a rectangular grid of rooms with sides of length n
//  and m
// . Each room has coordinates (a,b)
// , where 1≤a≤n,1≤b≤m
// . Thus we call a hall with coordinates (c,d)
//  a neighbouring for it if |a−c|+|b−d|=1
// .
...
// Currently, Vika is in a room with coordinates (x,y)
// , and her k
//  friends are in rooms with coordinates (x1,y1)
// , (x2,y2)
// , ... ,(xk,yk)
// , respectively. The coordinates can coincide. Note that all the girls must move to the neighbouring rooms.
...
// , k
//  (1≤n,m,k≤100
// ) — the sizes of the mall and the number of Vika's friends.

// The second line of each test case contains a pair of integers x
//  and y
//  (1≤x≤n
// , 1≤y≤m
// ) — the coordinates of the room where Vika is.

// Each of the next k
//  lines of each test case contains a pair of integers xi
//  and yi
//  (1≤xi≤n
// , 1≤yi≤m
// ) — the coordinates of the room where the i
// -th friend is.
...
#define in_1D_arr(a,n)                     FOR(i,0,n){cin>>a[i];}
#define in_2D_arr(a,n,m)                   FOR(i,0,n){FOR(j,0,m){cin>>a[i][j];}}
#define in_1D_vect(v,n)                    FOR(i,0,n){cin>>v[i];}
#define in_2D_vect(v,n,m)                  FOR(i,0,n){FOR(j,0,m){cin>>v[i][j];}}
