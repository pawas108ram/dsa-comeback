...
long long int n, m;
cin >> n >> m;
vector<long long int> original(n,0);
vector<long long int> jealous(m, 0);
...
sort(jealous.rbegin(), jealous.rend());
int left1 = 0;
int right1 = m - 1;
int left2 = 0;
int right2 = n-1;
...
vector<int> newarr;
while(n--){
    long long int rightdiff = abs(original[right2] - jealous[right1]);
    long long int leftdiff = abs(original[left2] - jealous[left1]);
    if(leftdiff>rightdiff){

        diff += leftdiff;
        cout << original[left1] << " " << original[left2] << endl;
        left1++;
        left2++;
    }
    else{
        diff += rightdiff;
         cout << original[right1] << " " << original[right2] << endl;
        right2--;
        right1--;
